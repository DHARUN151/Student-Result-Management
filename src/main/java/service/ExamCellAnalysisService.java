...
List<Double> values=dao.getDepartmentPercentages(departmentId);
...
result.put("distribution",new int[]{0,0,0,0,0});
return result;
}
double sum=0;
...
int[] distribution=new int[]{0,0,0,0,0};
for(double value:values){
double z=0;
if(standardDeviation>0)z=(value-mean)/standardDeviation;
...
if(departmentId<=0||userId<=0)return false;
if(remarks==null)remarks="";
remarks=remarks.trim();
if(remarks.length()>500)remarks=remarks.substring(0,500);
return dao.approveDepartmentResults(departmentId,userId,remarks);
}
...
remarks=remarks.trim();
if(remarks.length()>500)remarks=remarks.substring(0,500);
return dao.publishDepartmentResults(departmentId,userId,remarks);
}
}